...
def plot_skin_care_genero(df: pd.DataFrame, contx: st):
    df_sc = df.groupby(['genero', 'rotina_skin_care']).size().unstack().fillna(0)
    # Reorganize os dados para criar o gráfico
    df_sc = df_sc.reset_index()
    df_sc = pd.melt(df_sc, id_vars='genero', var_name='Rotina de Skin Care', value_name='Contagem')
...
def plot_idade_pesquisados(df: pd.DataFrame, contx: list):
    ...
    fig = px.histogram(df, x='idade', title="Distribuição de Idades dos Respondentes")
...
def plot_troca_escova_pesquisados(df: pd.DataFrame, contx: st):
    ordem = ['Todo mês', 'A cada 3 meses', 'A cada 6 meses', '1 vez por ano ou mais']
    # Converter a coluna 'substitui_escova_dentes' em uma variável categórica
    df['substitui_escova_dentes'] = df['substitui_escova_dentes'].astype('category')
    agrupado = df.groupby('substitui_escova_dentes').size().reset_index(name='Contagem')
    fig = px.bar(agrupado, x='substitui_escova_dentes', y='Contagem', title="Substituição escova", barmode='group')
