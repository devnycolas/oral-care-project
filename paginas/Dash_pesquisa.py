...
ordem = ["Menos de R$ 1.000,00", "R$ 1.000,00 a R$ 2.000,00", "R$ 2.001,00 a R$ 3.000,00",
         "R$ 3.001,00 a R$ 4.000,00", "Mais de R$ 4.000,00", "Não quero responder"]
...
def plot_renda_conhece_oc(df: pd.DataFrame, contx: st):
    ...
    df['conhece_oral_care_bin'] = (
        df['conhece_oral_care'] == 'Sim').astype(int)
    # Use groupby e count para contar "Sim" e "Não" separadamente
    df_grouped = df.groupby(
        ['renda_percapita', 'conhece_oral_care']).size().unstack(fill_value=0)
    # Renomeie as colunas
    df_grouped = df_grouped.rename(
        columns={'Sim': 'Total Sim', 'Não': 'Total Não'})
    ...
    fig.update_traces(text=df_grouped['Total Sim'], textangle=0,
                      textposition='inside', selector=dict(name='Total Sim'))
    fig.update_traces(text=df_grouped['Total Não'], textangle=0,
                      textposition='inside', selector=dict(name='Total Não'))
...
def df_com_agravante(df: pd.DataFrame):
    col_positivos = ['habito_fumar', 'pratica_raspagem_lingua', 'frequencia_escova_dentes_diaria', 'uso_fio_dental',
                     'substitui_escova_dentes', 'usa_enxaguante_bucal', 'usa_protetor_solar_labial', 'uso_frequente_hidratante_labial']
    ...
    df[col_negativos] = df[col_negativos].map(
        lambda x: mapeamento_negativo.get(x, 0))

    # Soma dos resultados
    df['agravantes'] = df[col_positivos].sum(
        axis=1) + df[col_negativos].sum(axis=1)
...
def plot_agravantes(df: pd.DataFrame, contx: st):
    ...
    df_grouped = df_agravante.groupby(
        ['renda_percapita', 'agravantes']).size().unstack(fill_value=0)
    ordem_desejada = ["excelente", "muito bom",
                      "bom", "ruim", "muito ruim", "péssimo"]
    ...
    fig = px.bar(df_grouped, x=df_grouped.index, y=df_grouped.columns, title='Relação entre renda e cuidados com a boca',
                 labels={'index': 'Renda Per Capita',
                         'value': 'Quantidade', 'classificacao': 'condição'},
...
def plot_renda_conhece_produtos_oc(df: pd.DataFrame, contx: st):
    ...
    # Crie uma coluna auxiliar com 1 para "Sim" e 0 para "Não"
    df['conhece_oral_care_bin'] = (
        df['produtos_relacionados_oral_care'] == 'Sim').astype(int)
    # Use groupby e count para contar "Sim" e "Não" separadamente
    df_grouped = df.groupby(
        ['renda_percapita', 'produtos_relacionados_oral_care']).size().unstack(fill_value=0)
    # Renomeie as colunas
    df_grouped = df_grouped.rename(
        columns={'Sim': 'Total Sim', 'Não': 'Total Não'})
    # Reset o índice, se desejar
...
def plot_skin_oc(df: pd.DataFrame, contx: st):
    ...
    porcentagem_sim_nao = df_filtrado['conhece_oral_care'].value_counts(
        normalize=True) * 100
...
def plot_mapa(df: pd.DataFrame, contx: st):
    selected_city = contx[1].selectbox(
        'conhece sobre oral care', df['conhece_oral_care'].unique())

    # Filtrar o DataFrame com base na seleção da cidade
    filtered_df = df_com_coords(df)
    filtered_df = filtered_df[filtered_df['conhece_oral_care']
                              == selected_city]
...
def plot_conhece_oral_care_genero(df: pd.DataFrame, contx: st):
    ...
    df_group = df.groupby(['genero', 'conhece_oral_care']
                          ).size().unstack().fillna(0)
    # Reorganize os dados para criar o gráfico
    df_group = df_group.reset_index()
    df_group = pd.melt(df_group, id_vars='genero',
                       var_name='Conhece Oral Care', value_name='Contagem')
...
def plot_conhece_oral_care_cidade(df: pd.DataFrame, contx: st):
    # Calcule as contagens por gênero e se conhecem o Oral Care
    df_group = df.groupby(['cidade', 'conhece_oral_care']
                          ).size().unstack().fillna(0)
    # Reorganize os dados para criar o gráfico
    df_group = df_group.reset_index()
    df_group = pd.melt(df_group, id_vars='cidade',
                       var_name='Conhece Oral Care', value_name='Contagem')
    # Organize os dados em ordem crescente de contagem
    df_group = df_group.sort_values(by='Contagem', ascending=False)
    # Defina uma paleta de cores personalizada
